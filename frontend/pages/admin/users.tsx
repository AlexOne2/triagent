  ...
  return (
    <main className="full">
      <header>
        <div>
          <h1>Admin - Users</h1>
          <p>Manage local users and role assignments.</p>
        </div>
      </header>
      <AdminNav />
            ...
            <div key={user.id} className="admin-user-row">
                ...
                {roleKeys.map((roleKey) => (
                  <label key={`${user.id}-${roleKey}`} className="admin-checkbox">
                    <input
                      ...
                      onChange={() => setDrafts((current) => ({ ...current, [user.id]: { ...draft, roleKeys: toggleRole(draft.roleKeys, roleKey) } }))}
                      disabled={!canWrite}
                    />
                    {roleKey}
                  </label>
              ...
              {canWrite ? (
                <div className="admin-actions">
                  <button className="tab" type="button" onClick={() => void saveProfile(user)} disabled={savingUserId === user.id}>
                    Save Profile
                  </button>
